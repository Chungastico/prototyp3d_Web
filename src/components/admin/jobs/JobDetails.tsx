...
export function JobDetails({ jobId, onBack }: JobDetailsProps) {
    ...
    const fetchData = useCallback(async () => {
        ...
        // Fetch Job, Pieces, and Job-level Extras concurrently
        const [
            { data: jobData },
            { data: piecesData },
            { data: extrasData }
        ...
        // Identify secondary data to fetch (filaments, piece-level extras)
        const filamentIds = piecesData && piecesData.length > 0
            ? Array.from(new Set(piecesData.map(p => p.filamento_id).filter(Boolean)))
            : [];
            
        const pieceIds = piecesData && piecesData.length > 0
            ? piecesData.map(p => p.id)
            : [];

        // Fetch piece-level extras and filaments concurrently
        const [
            { data: filaments },
            { data: pieceExtras }
        ] = await Promise.all([
        ...
        if (pieceExtras) {
            allExtras = [...allExtras, ...pieceExtras];
        }

        // Fetch Extra Names based on allExtras
        const extraIds = allExtras.length > 0
            ? Array.from(new Set(allExtras.map(e => e.extra_id)))
            : [];
