...
export default function StudentCatalogPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-8rem)] bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative">
        ...
        <div className="md:hidden flex-none p-4 pb-2 bg-white z-20 border-b border-gray-100">
             <div className="flex gap-2">
                 <div className="relative flex-1">
                    <input 
                        type="text"
                        placeholder="Buscar en el catálogo..."
                        className="w-full pl-9 pr-4 py-2.5 rounded-xl text-gray-800 bg-gray-50 border-gray-200 focus:ring-2 focus:ring-naranja/20 text-sm"
                        value={searchQuery}
        ...
        <aside className="hidden md:flex w-72 flex-shrink-0 bg-gray-50/50 border-r border-gray-100 flex-col h-full overflow-hidden">
            <div className="p-6 overflow-y-auto h-full space-y-8 scrollbar-thin">
                ...
                <div className="relative">
                    <input 
                        type="text"
                        placeholder="Buscar..."
                        className="w-full pl-10 pr-4 py-3 rounded-xl text-gray-800 bg-white border border-gray-200 focus:outline-none focus:ring-2 focus:ring-naranja/20 focus:border-naranja shadow-sm text-sm transition-all"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                    />
                ...
                <div>
                    <button 
                        ...
                        className="flex items-center justify-between w-full font-bold text-gray-900 text-sm mb-4 hover:text-naranja transition-colors"
                    >
                ...
                <div>
                     <div className="flex justify-between items-center mb-4">
                        <button 
                            ...
                            className="flex items-center justify-between w-full font-bold text-gray-900 text-sm hover:text-naranja transition-colors"
                        >
                             <span>Etiquetas</span>
        ...
        <main className="flex-1 overflow-y-auto h-full bg-white relative scroll-smooth w-full p-4 md:p-8">
            <h1 className="text-2xl font-bold text-gray-900 mb-6 hidden md:block">Explora nuestro Catálogo</h1>
            
            {/* Active Filters Summary (Mobile/Desktop) */}
            {(selectedCategory || selectedTags.length > 0 || (searchQuery && window.innerWidth >= 768)) && (
                <div className="mb-6 flex flex-wrap gap-2 items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <span className="text-sm text-gray-500 mr-2 font-bold">Filtros:</span>
                    {selectedCategory && selectedCategory !== "Todos" && (
                            <Badge className="bg-naranja text-white text-xs px-2 py-0.5">{selectedCategory}</Badge>
                    )}
                    {selectedTags.map(tag => (
                            <Badge key={tag} className="bg-azul-oscuro text-white text-xs px-2 py-0.5">{tag} <X onClick={() => toggleTag(tag)} className="ml-1 h-3 w-3 cursor-pointer"/></Badge>
                    ))}
                        <button 
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 pb-20">
                            ...
                            >
                                ...
                                <div className="relative aspect-square overflow-hidden bg-gray-50 flex items-center justify-center p-4">
                                    <div className="absolute top-3 left-3 z-10">
                                            <Badge variant="secondary" className="bg-white/90 backdrop-blur-sm text-gray-900 font-semibold shadow-sm px-2.5 py-1 text-xs">
                                            {item.category}
                                        </Badge>
                                    </div>
                                ...
                                <div className="p-4 md:p-5 flex flex-col flex-1">
                                    <h3 className="font-bold text-gray-900 mb-1 leading-tight group-hover:text-naranja transition-colors">{item.title}</h3>
                                    <p className="line-clamp-2 text-gray-500 text-sm mb-4 leading-relaxed">{item.description}</p>
                                    
                                    <div className="mt-auto pt-4 border-t border-gray-50 flex items-center gap-2 overflow-hidden flex-wrap">
                                        {item.tags?.slice(0, 2).map((tag) => (
                                            <span key={tag} className="text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded-md border border-gray-100 whitespace-nowrap">
                                                #{tag}
                                            </span>
                                        ))}
                                    </div>
                    ...
                    <p className="text-gray-500 max-w-sm mx-auto">Intenta ajustar tu búsqueda o limpiar los filtros seleccionados.</p>
                    <Button onClick={() => { setSelectedTags([]); setSelectedCategory(null); setSearchQuery(''); }} variant="outline" className="mt-6">
